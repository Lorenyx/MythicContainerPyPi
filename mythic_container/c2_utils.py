...
from .MythicGoRPC.send_mythic_rpc_c2_update_status import *
...
async def keep_reading_stdout(c2_profile: str):
    if "reading" in C2ProfileBase.runningServers[c2_profile]:
        return
    while True:
        C2ProfileBase.runningServers[c2_profile]["reading"] = 1
        try:
            line = await asyncio.wait_for(
                C2ProfileBase.runningServers[c2_profile]["process"].stdout.readline(), timeout=3.0)
            if line is not None:
                if len(line) == 0:
                    # the process has stopped, but our loop will keep returning blank lines
                    if C2ProfileBase.runningServers[c2_profile]["process"].returncode is not None:
                        del C2ProfileBase.runningServers[c2_profile]["reading"]
                        return
                    continue
                logger.debug(line.decode())
                if "output" in C2ProfileBase.runningServers[c2_profile]:
                    C2ProfileBase.runningServers[c2_profile]["output"].append(line.decode())
                else:
                    C2ProfileBase.runningServers[c2_profile]["output"] = deque([line.decode()], 100)
        except TimeoutError:
...
async def deal_with_stdout(c2_profile: str) -> str:
    ...
    if "output" in C2ProfileBase.runningServers[c2_profile]:
        try:
            while True:
                output += C2ProfileBase.runningServers[c2_profile]["output"].popleft()
        except IndexError:
            pass
        except Exception as e:
...
async def opsecChecks(msg: bytes) -> bytes:
    try:
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if callable(c2.opsec):
                    try:
                        result = await c2.opsec(C2ProfileBase.C2OPSECMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call opsec for {c2.name}")
                        response = C2ProfileBase.C2OPSECMessageResponse(
                            Success=False,
                    ...
                    if result is None:
                        ...
                        response = C2ProfileBase.C2OPSECMessageResponse(**result)
                    ...
                    elif isinstance(result, C2ProfileBase.C2OPSECMessageResponse):
    ...
    try:
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if callable(c2.opsec):
                    try:
                        result = await c2.config_check(C2ProfileBase.C2ConfigCheckMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call config check for {c2.name}")
                        response = C2ProfileBase.C2ConfigCheckMessageResponse(
                            Success=False,
                    ...
                    if result is None:
                        ...
                        response = C2ProfileBase.C2ConfigCheckMessageResponse(**result)
                        if response.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(response).encode()
                    elif isinstance(result, C2ProfileBase.C2ConfigCheckMessageResponse):
                        if result.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(result.to_json()).encode()
                    else:
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if callable(c2.get_ioc):
                    try:
                        result = await c2.config_check(C2ProfileBase.C2GetIOCMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call get_ioc for {c2.name}")
                        response = C2ProfileBase.C2GetIOCMessageResponse(
                            Success=False,
                            Error=f"Failed to call config check function: {traceback.format_exc()}\n{callEx}"
                        )
                        return ujson.dumps(response.to_json()).encode()
                    if result is None:
                        ...
                        response = C2ProfileBase.C2GetIOCMessageResponse(**result)
                        if response.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(response).encode()
                    elif isinstance(result, C2ProfileBase.C2GetIOCMessageResponse):
                        if result.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(result.to_json()).encode()
                    else:
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if callable(c2.sample_message):
                    try:
                        result = await c2.config_check(C2ProfileBase.C2SampleMessageMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call get_ioc for {c2.name}")
                        response = C2ProfileBase.C2SampleMessageMessageResponse(
                            Success=False,
                    ...
                    if result is None:
                        ...
                        response = C2ProfileBase.C2SampleMessageMessageResponse(**result)
                        if response.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(response).encode()
                    elif isinstance(result, C2ProfileBase.C2SampleMessageMessageResponse):
                        if result.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(result.to_json()).encode()
                    else:
...
async def getDebugOutput(msg: bytes) -> bytes:
    try:
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if C2ProfileBase.runningServers[c2.name]["process"] is not None:
                    if C2ProfileBase.runningServers[c2.name]["process"].returncode is None:
                        running = True
                    else:
                        running = False
                        try:
                            kill(C2ProfileBase.runningServers[c2.name]["process"].pid)
                        except Exception as e:
                            pass
                    response = C2ProfileBase.C2GetDebugOutputMessageResponse(
                ...
                if callable(c2.redirect_rules):
                    try:
                        result = await c2.redirect_rules(
                            C2ProfileBase.C2GetRedirectorRulesMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call get redirector rules for {c2.name}")
                        response = C2ProfileBase.C2GetRedirectorRulesMessageResponse(
                            Success=False,
                    ...
                    if result is None:
                        ...
                        response = C2ProfileBase.C2GetRedirectorRulesMessageResponse(**result)
                        if response.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(response).encode()
                    elif isinstance(result, C2ProfileBase.C2GetRedirectorRulesMessageResponse):
                        if result.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(result.to_json()).encode()
                    else:
...
async def startServerBinary(
        c2: C2ProfileBase.C2Profile) -> C2ProfileBase.C2StartServerMessageResponse:
    try:
        if c2.name not in C2ProfileBase.runningServers:
            C2ProfileBase.runningServers[c2.name] = {
                "process": None
            }
...
async def startServer(msg: bytes) -> bytes:
    with c2Mutex:
        try:
            ...
            for name, c2 in C2ProfileBase.c2Profiles.items():
                if c2.name == msgDict["c2_profile_name"]:
                    if c2.name in C2ProfileBase.runningServers:
                        # we've at least started this before, so we can check some keys
                        if C2ProfileBase.runningServers[c2.name]["process"] is None:
                            # process isn't running, so we need to start it
                            response = await startServerBinary(c2)
                            return ujson.dumps(response.to_json()).encode()

                        elif C2ProfileBase.runningServers[c2.name]["process"].returncode is None:
                            response = C2ProfileBase.C2StartServerMessageResponse(
            ...
            for name, c2 in C2ProfileBase.c2Profiles.items():
                if c2.name == msgDict["c2_profile_name"]:
                    if c2.name in C2ProfileBase.runningServers:
                        ...
                        if C2ProfileBase.runningServers[c2.name]["process"] is None:
                            # process isn't running, so we need to start it
                            response = C2ProfileBase.C2StopServerMessageResponse(
                                Success=True, Error="", Message=f"The server is not running",
                                InternalServerRunning=False
                            )
                            return ujson.dumps(response.to_json()).encode()

                        elif C2ProfileBase.runningServers[c2.name]["process"].returncode is not None:
                            ...
                            if "output" in C2ProfileBase.runningServers[c2.name]:
                                C2ProfileBase.runningServers[c2.name]["output"].clear()
                            return ujson.dumps(response.to_json()).encode()
                    else:
                        # just means we've never started it before, so start it now
                        response = C2ProfileBase.C2StopServerMessageResponse(
...
async def customRPCFunction(msg: bytes) -> bytes:
    try:
        msgDict = ujson.loads(msg)
        incomingMessage = C2ProfileBase.C2OtherServiceRPCMessage(**msgDict)
        for name, c2 in C2ProfileBase.c2Profiles.items():
                    ...
                    for key, f in c2.custom_rpc_functions.items():
                        if key == incomingMessage.ServiceRPCFunction:
                            ...
                            if response.RestartInternalServer:
                                t = asyncio.create_task(restartInternalServer(name=c2.name))
                            return ujson.dumps(response.to_json()).encode()
                    response = C2ProfileBase.C2OtherServiceRPCMessageResponse(
                        Success=False,
        ...
        for name, c2 in C2ProfileBase.c2Profiles.items():
            if c2.name == msgDict["c2_profile_name"]:
                if callable(c2.host_file):
                    try:
                        result = await c2.host_file(C2ProfileBase.C2HostFileMessage(**msgDict))
                    except Exception as callEx:
                        logger.exception(
                            f"Failed to call host_file for {c2.name}")
                        response = C2ProfileBase.C2HostFileMessageResponse(
                            Success=False,
                            Error=f"Failed to call config check function: {traceback.format_exc()}\n{callEx}"
                        )
                        return ujson.dumps(response.to_json()).encode()
                    if result is None:
                        ...
                        response = C2ProfileBase.C2HostFileMessageResponse(**result)
                        if response.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(response).encode()
                    elif isinstance(result, C2ProfileBase.C2HostFileMessageResponse):
                        if result.RestartInternalServer:
                            t = asyncio.create_task(restartInternalServer(name=c2.name))
                        return ujson.dumps(result.to_json()).encode()
                    else:
